...
// [[Rcpp::export]]
SEXP fit_model_Indep_aggToTime_internal(int E, int BURN_IN, int n_Data, double a_phi,double a_sigma, double b_sigma, double b_phi, double a_omega, double b_omega, 
                                              SEXP ms_r, SEXP phi_0_r, SEXP phi_1_r, SEXP lambda_0_r, SEXP lambda_1_r, SEXP V_0_r, SEXP V_1_r, SEXP omega_0_r, SEXP omega_1_r,
                                              SEXP X_r, SEXP D_r, SEXP Yt_r, SEXP H_r, int K_r, double sigma2_pi_r,  SEXP sub_size_n_r) {
  ...
  for(itr = 0; itr < E; itr++){
    ...
    // sample ms
    inv_Sigma_pi = create_inv_Sigma_pi(sigma2_pi, n_Data);
    for(i = 0; i<n_Data; i++){
      mu_m(i) = 0;
    }
    ...
    for (int i = 0; i < n_Data; i++) {
      sum_pis_sq += pow(ms(i), 2);
    }
    sigma2_pi = 1 / Rcpp::rgamma(1, a_sigma + n_Data/2, 1 / (b_sigma + sum_pis_sq/2))(0);
